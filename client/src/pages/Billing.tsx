...
export default function Billing() {
  ...
  const { data: summary, isLoading: summaryLoading } = useQuery<BillingSummary>(
    {
      ...
      queryFn: async () => {
        const res = await fetch(
          `/api/billing/summary?from=${encodeURIComponent(dateParams.from)}&to=${encodeURIComponent(dateParams.to)}`,
          {
            credentials: "include",
          },
        );
  ...
  >({
    ...
    queryFn: async () => {
      const res = await fetch("/api/billing/revenue-trend", {
        credentials: "include",
      });
  ...
  const getPaymentTypeBadge = (type: string) => {
    ...
    return (
      <Badge variant="outline" data-testid={`badge-payment-type-${type}`}>
  ...
  const typeChartData =
    summary?.paymentTypeBreakdown?.map((t) => ({
    ...
    <div className="space-y-6 p-5 pb-20">
      ...
      {!canEdit && (
        <Alert data-testid="alert-read-only">
          ...
          <AlertDescription>
            ...
            create or edit payments and invoices.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card data-testid="card-partial-collected">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(summary?.partialPaymentsCollected || 0)}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              {summary?.partiallyPaidCount || 0} partial payments
            </p>
          </CardContent>
        </Card>
        ...
        <Card data-testid="card-total-dues">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              {formatCurrency(summary?.totalDues || 0)}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
        ...
        <Card data-testid="card-avg-ticket">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(summary?.avgTicketSize || 0)}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              Per transaction
            </p>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card data-testid="card-membership-revenue">
          ...
          <CardContent>
            <div className="text-xl font-bold">
              {formatCurrency(summary?.membershipRevenue || 0)}
            </div>
          </CardContent>
        </Card>
        ...
        <Card data-testid="card-shop-revenue">
          ...
          <CardContent>
            <div className="text-xl font-bold">
              {formatCurrency(summary?.shopRevenue || 0)}
            </div>
          </CardContent>
        </Card>
      </div>
                  ...
                  {paymentsLoading ? (
                    ...
                    filteredPayments.map((payment) => (
                      ...
                      >
                        ...
                        <TableCell>
                          {payment.amountDue &&
                          parseFloat(String(payment.amountDue)) > 0 ? (
                            <span className="text-red-600 font-medium">
                              {formatCurrency(
                                parseFloat(String(payment.amountDue)),
                              )}
                            </span>
                          ) : (
