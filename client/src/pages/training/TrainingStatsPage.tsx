...
export default function TrainingStatsPage() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 p-4 sm:p-6 pb-20">
      {/* Page Header */}
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-white">Statistics</h1>
        <p className="text-sm text-white/60">Track your training progress</p>
      </div>
      ...
      <Card className="bg-gradient-to-br from-slate-800 to-slate-900 border-white/10 overflow-hidden">
        <CardContent className="p-4 sm:p-6">
          ...
          <div className="h-20 sm:h-24 flex items-end justify-between gap-2 mb-4">
        ...
        <div className="space-y-3">
          {muscleProgress.length === 0 ? (
            ...
            muscleProgress.map((muscle) => {
              const percentage = Math.min(100, Math.round((parseInt(String(muscle.workout_count)) / 10) * 100));
              return (
                <Card key={muscle.target_muscle} className="bg-white/5 border-white/10">
                  <CardContent className="p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-rose-400 to-rose-500 flex items-center justify-center">
        ...
        <div className="space-y-3">
          <Card className="bg-white/5 border-white/10">
            <CardContent className="p-4 flex items-center gap-4">
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-rose-400 to-rose-500" />
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold truncate">Weekly Workouts</h3>
                <p className="text-sm text-white/50">{stats?.weeklyWorkouts || 0} sessions</p>
              </div>
              <span className="text-xs sm:text-sm text-white/40 flex-shrink-0">This week</span>
            </CardContent>
          </Card>
          <Card className="bg-white/5 border-white/10">
            <CardContent className="p-4 flex items-center gap-4">
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-orange-400 to-orange-500 flex items-center justify-center">
                <Flame className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold truncate">Calories Burned</h3>
          ...
          <Card className="bg-white/5 border-white/10">
            <CardContent className="p-4 flex items-center gap-4">
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-cyan-400 to-cyan-500 flex items-center justify-center">
                <TrendingUp className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold truncate">Current Streak</h3>
                <p className="text-sm text-white/50">{stats?.currentStreak || 0} days</p>
              </div>
              <span className="text-xs sm:text-sm text-white/40 flex-shrink-0">Keep it up!</span>
            </CardContent>
          </Card>
        </div>
      </div>
