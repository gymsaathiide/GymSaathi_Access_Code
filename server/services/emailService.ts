...
export interface NewBranchNotificationPayload {
...
export async function sendNewBranchNotificationEmail(payload: NewBranchNotificationPayload): Promise<boolean> {
  if (!resend) {
    console.log('[email] RESEND_API_KEY not configured, skipping new branch notification email');
    return false;
  }
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>New Branch Created</title>
</head>
<body style="margin: 0; padding: 0; background-color: #f3f4f6; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
    <tr>
      <td style="background: linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%); padding: 30px 20px; text-align: center;">
        <h1 style="color: #ffffff; margin: 0; font-size: 24px;">🏢 New Branch Created</h1>
        <p style="color: #ffffff; margin: 10px 0 0; opacity: 0.9;">GYMSAATHI Admin Notification</p>
      </td>
    </tr>
    <tr>
      <td style="padding: 30px;">
        ...
        <p style="color: #374151; font-size: 16px; line-height: 1.6; margin: 0 0 20px;">
          A new branch has been created under <strong>${payload.parentGymName}</strong>.
        </p>
        
        <div style="margin: 25px 0; padding: 20px; background-color: #eff6ff; border-radius: 12px; border-left: 4px solid #3b82f6;">
          <h3 style="color: #1e40af; margin: 0 0 15px;">Branch Details</h3>
          <table style="width: 100%; border-collapse: collapse;">
            <tr>
              <td style="color: #6b7280; padding: 8px 0;">Branch Name:</td>
              <td style="color: #374151; font-weight: 600; padding: 8px 0;">${payload.branchName}</td>
            </tr>
            <tr>
              <td style="color: #6b7280; padding: 8px 0;">Parent Gym:</td>
              <td style="color: #374151; font-weight: 600; padding: 8px 0;">${payload.parentGymName}</td>
            </tr>
            <tr>
              <td style="color: #6b7280; padding: 8px 0;">Created By:</td>
              <td style="color: #374151; font-weight: 600; padding: 8px 0;">${payload.adminName}</td>
            </tr>
            ${payload.branchAddress ? `
            <tr>
              <td style="color: #6b7280; padding: 8px 0;">Address:</td>
              <td style="color: #374151; padding: 8px 0;">${payload.branchAddress}</td>
            </tr>` : ''}
            ${payload.branchPhone ? `
            <tr>
              <td style="color: #6b7280; padding: 8px 0;">Phone:</td>
              <td style="color: #374151; padding: 8px 0;">${payload.branchPhone}</td>
            </tr>` : ''}
          </table>
        </div>
        
        <p style="color: #6b7280; font-size: 14px; line-height: 1.6; margin: 25px 0 0;">
          This notification was automatically sent from GYMSAATHI when an admin created a new branch.
        </p>
      </td>
    </tr>
    <tr>
    ...
    const { data, error } = await resend.emails.send({
      ...
      subject: `New Branch Created: ${payload.branchName} under ${payload.parentGymName}`,
      html,
      text: `Hi ${payload.superadminName}, A new branch "${payload.branchName}" has been created under ${payload.parentGymName} by ${payload.adminName}.${payload.branchAddress ? ` Address: ${payload.branchAddress}.` : ''}${payload.branchPhone ? ` Phone: ${payload.branchPhone}.` : ''}`,
    });

    if (error) {
      console.error('[email] ERROR sending new branch notification email:', error);
      return false;
    }

    console.log(`[email] Sent new branch notification email to ${payload.superadminEmail} (ID: ${data?.id})`);
    return true;
  } catch (error) {
    console.error('[email] ERROR sending new branch notification email:', error);
    return false;
