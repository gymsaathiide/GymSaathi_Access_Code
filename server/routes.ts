...
Return ONLY the JSON object, no other text.`;
  ...
  // Bulk import meals for breakfast (Excel import)
  app.post('/api/meals/breakfast/import', async (req, res) => {
    if (!req.session?.userId) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
    ...
    try {
      ...
      console.error('Error importing breakfast meals:', error);
      res.status(500).json({ error: error.message });
    }
  });

  // Bulk import meals for snacks (Excel import)
  app.post('/api/meals/snacks/import', async (req, res) => {
    if (!req.session?.userId) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
    ...
    try {
      ...
      for (const meal of meals) {
        try {
          ...
          await db!.execute(sql`
            INSERT INTO meals_snacks (name, description, ingredients, protein, carbs, fats, calories, category)
            VALUES (
              ${name.trim()}, 
              ${description || null}, 
              ${ingredients || null}, 
      ...
      console.error('Error importing snacks meals:', error);
      res.status(500).json({ error: error.message });
    }
  });

  // Bulk import meals for lunch (Excel import)
  app.post('/api/meals/lunch/import', async (req, res) => {
    if (!req.session?.userId) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
    ...
    try {
      ...
      for (const meal of meals) {
        try {
          ...
          await db!.execute(sql`
            INSERT INTO meals_lunch (name, description, ingredients, protein, carbs, fats, calories, category)
            VALUES (
              ${name.trim()}, 
              ${description || null}, 
              ${ingredients || null}, 
      ...
      console.error('Error importing lunch meals:', error);
      res.status(500).json({ error: error.message });
    }
  });
  ...
  app.post('/api/meals/breakfast/generate-plan', async (req, res) => {
    ...
    try {
      ...
      // Cumulative category filtering (union-based, not proportional):
      // - 'veg' = only vegetarian meals
      // - 'eggetarian' = vegetarian + egg-based meals (union)
      // - 'non-veg' or 'all' = vegetarian + egg-based + non-vegetarian meals (all)
      // 
      // Each tier includes ALL meals from lower tiers. Meals are selected randomly
      // from the combined pool, not proportionally distributed across categories.
      
      // Normalize 'all' to 'non-veg' for consistent handling
      const normalizedCategory = category === 'all' ? 'non-veg' : category;
      
      // Build the WHERE clause based on cumulative categories
      let mealsQuery;
      if (normalizedCategory === 'veg') {
        mealsQuery = await db!.execute(sql`
          SELECT * FROM meals_breakfast 
          WHERE category = 'veg' 
          ORDER BY RANDOM()
        `);
      } else if (normalizedCategory === 'eggetarian') {
        mealsQuery = await db!.execute(sql`
          SELECT * FROM meals_breakfast 
        ...
        // 'non-veg' includes all categories
        mealsQuery = await db!.execute(sql`
          SELECT * FROM meals_breakfast 
