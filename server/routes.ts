...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // GET /api/branches - Get all branches for the current gym (admin only)
  app.get('/api/branches', requireAuth, async (req, res) => {
    try {
      ...
      if (user.role !== 'admin') {
        return res.status(403).json({ error: 'Only admins can view branches' });
      }
      ...
      const gymAdmin = await db!.select().from(schema.gymAdmins).where(eq(schema.gymAdmins.userId, user.id)).limit(1).then(rows => rows[0]);
      if (!gymAdmin?.gymId) {
        return res.status(400).json({ error: 'Admin not associated with a gym' });
      }

      const parentGym = await db!.select().from(schema.gyms).where(eq(schema.gyms.id, gymAdmin.gymId)).limit(1).then(rows => rows[0]);
      if (!parentGym) {
        return res.status(404).json({ error: 'Gym not found' });
      }

      // Get all branches for this gym (gyms where parentGymId matches the admin's gym)
      const branches = await db!.select()
        .from(schema.gyms)
        .where(eq(schema.gyms.parentGymId, gymAdmin.gymId))
        .orderBy(asc(schema.gyms.name));

      // Include the main gym as well
      const allBranches = [
        { id: parentGym.id, name: parentGym.name, isMain: true, address: parentGym.address, phone: parentGym.phone },
        ...branches.map(b => ({ id: b.id, name: b.name, isMain: false, address: b.address, phone: b.phone }))
      ];
  ...
  app.post('/api/branches', requireAuth, async (req, res) => {
    try {
      ...
      const parentGym = await db!.select().from(schema.gyms).where(eq(schema.gyms.id, gymAdmin.gymId)).limit(1).then(rows => rows[0]);
      if (!parentGym) {
        return res.status(404).json({ error: 'Parent gym not found' });
      }

      // Generate a unique slug for the branch
      const baseSlug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
      const timestamp = Date.now().toString(36);
      const slug = `${baseSlug}-${timestamp}`;

      // Create the branch as a new gym with parentGymId set
      const [newBranch] = await db!.insert(schema.gyms).values({
        name,
        slug,
        owner: parentGym.owner,
        email: parentGym.email,
        phone: phone || parentGym.phone,
        plan: parentGym.plan,
        status: 'active',
        address: address || null,
        parentGymId: parentGym.id,
      }).returning();

      // Create in-app notification for all superadmins
      ...
      for (const superadmin of superadmins) {
        ...
        await db!.insert(schema.notifications).values({
          ...
          message: `A new branch "${name}" was created under ${parentGym.name} by ${user.name}.`,
          type: 'info',
          isRead: 0,
        });
        ...
        // Send email notification
        if (superadmin.email) {
          sendNewBranchNotificationEmail({
            ...
            branchPhone: phone,
          }).catch(err => console.error('Failed to send branch notification email:', err));
        }
      }

      // Log the audit event
      await auditService.logBranchCreated(parentGym.id, user.id, user.name, name, newBranch.id);
